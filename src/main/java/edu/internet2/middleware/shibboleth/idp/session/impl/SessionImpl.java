...
public class SessionImpl extends AbstractSession implements Session {
    ...
    public SessionImpl(String sessionId, byte[] secret, long timeout) {
        ...
        authnMethods = new /*Concurrent*/HashMap<String, AuthenticationMethodInformation>(2);
        servicesInformation = new /*Concurrent*/HashMap<String, ServiceInformation>(2);
    ...
    public /*synchronized*/ Map<String, AuthenticationMethodInformation> getAuthenticationMethods() {
        return Collections.unmodifiableMap(authnMethods);
    }

    /** {@inheritDoc} */
    public /*synchronized*/ Map<String, ServiceInformation> getServicesInformation() {
        return Collections.unmodifiableMap(servicesInformation);
    }
    ...
    public /*synchronized*/ void addServicesInformation(String key, ServiceInformation value) {
        servicesInformation.put(key, value);
        changed = true;
    }
    public /*synchronized*/ void addAuthenticationMethods(String key, AuthenticationMethodInformation value){
        authnMethods.put(key, value);
        changed = true;
    }
    public /*synchronized*/ void remoteAuthenticationMethods(String key){
        authnMethods.remove(key);
        changed = true;
    }
    ...
    public /*synchronized*/ void markAsReplicated(){
        this.changed = false;
        this.replicationCount++;
        indexes.clear();
    }

    public /*synchronized*/ Set<String> getIndexes() {
        return Collections.unmodifiableSet(indexes);
    }

    public /*synchronized*/ boolean addIndex(String index) {
        return (index != null && indexes.add(index));
    }
