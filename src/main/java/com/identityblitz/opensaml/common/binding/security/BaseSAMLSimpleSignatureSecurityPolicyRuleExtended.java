package com.identityblitz.opensaml.common.binding.security;
...
/**
 */
public abstract class BaseSAMLSimpleSignatureSecurityPolicyRuleExtended implements SecurityPolicyRule {
    /** Logger. */
    private final Logger log = LoggerFactory.getLogger(BaseSAMLSimpleSignatureSecurityPolicyRuleExtended.class);
     ...
     */
    protected BaseSAMLSimpleSignatureSecurityPolicyRuleExtended(SignatureTrustEngine engine) {
        trustEngine = engine;
    }
    ...
    public void evaluate(MessageContext messageContext) throws SecurityPolicyException {
        ...
        if (!(messageContext instanceof SAMLMessageContext)) {
            log.error("Invalid message context type, this policy rule only supports SAMLMessageContext");
        ...
        if (!(messageContext.getInboundMessageTransport() instanceof HTTPInTransport)) {
            log.debug("Invalid inbound message transport type, this rule only supports HTTPInTransport");
            throw new SecurityPolicyException("Invalid inbound message transport type, this rule only supports HTTPInTransport");
        }

        SAMLMessageContext samlMsgCtx = (SAMLMessageContext) messageContext;
        HTTPInTransport itr = (HTTPInTransport)messageContext.getInboundMessageTransport();

        if (!ruleHandles(itr, samlMsgCtx)) {
            log.debug("Rule can not handle this request, skipping processing");
            return;
        }

        byte[] signature = getSignature(itr);
        if (signature == null || signature.length == 0) {
            log.warn("HTTP request was not signed via simple signature mechanism, skipping");
            return;
        }

        String sigAlg = getSignatureAlgorithm(itr);
        ...
        byte[] signedContent = getSignedContent(itr);
        ...
        doEvaluate(signature, signedContent, sigAlg, itr, samlMsgCtx);
    }
     ...
     * @param itr the inbount HTTP transport
     ...
     *
     */
    private void doEvaluate(byte[] signature, byte[] signedContent, String algorithmURI, HTTPInTransport itr,
                            SAMLMessageContext samlMsgCtx) throws SecurityPolicyException {

        List<Credential> candidateCredentials = getRequestCredentials(itr, samlMsgCtx);
     ...
     *
     * @param itr the inbound HTTP transport
     * @param samlContext the SAML message context being processed
     * @return a list of candidate validation credentials in the request, or null if none were present
     * @throws SecurityPolicyException thrown if there is an error during request processing
     */
    protected List<Credential> getRequestCredentials(HTTPInTransport itr, SAMLMessageContext samlContext)
            throws SecurityPolicyException {
     ...
     * {@link SignatureTrustEngine#validate(byte[], byte[], String, org.opensaml.xml.security.CriteriaSet, org.opensaml.xml.security.credential.Credential)}.
     *
     ...
     * @throws SecurityPolicyException thrown if there is an error during request processing
     */
    protected byte[] getSignature(HTTPInTransport itr) throws SecurityPolicyException {
        String signature = itr.getParameterValue("Signature");
     ...
     * @throws SecurityPolicyException thrown if there is an error during request processing
     */
    protected String getSignatureAlgorithm(HTTPInTransport itr) throws SecurityPolicyException {
        return itr.getParameterValue("SigAlg");
    }
     ...
     * @throws SecurityPolicyException thrown if there is an error during request processing
     */
    protected abstract byte[] getSignedContent(HTTPInTransport itr) throws SecurityPolicyException;
     ...
     * @throws SecurityPolicyException thrown if there is an error during request processing
     */
    protected abstract boolean ruleHandles(HTTPInTransport itr, SAMLMessageContext samlMsgCtx) throws SecurityPolicyException;
}
